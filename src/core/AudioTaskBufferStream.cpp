...
size_t AudioTaskBufferStream::write(AudioTaskBuffer buffer) {
...
size_t AudioTaskBufferStream::try_write(AudioTaskBuffer buffer) {
    if (buffer.empty()) return 0;
    const size_t bytes = buffer.size();
...
void AudioTaskBufferStream::end_input() noexcept {
    std::lock_guard<std::mutex> lock(_mutex);
    _finished = true;
...
AudioTaskBuffer AudioTaskBufferStream::try_pop() {
    std::lock_guard<std::mutex> lock(_mutex);
...
void AudioTaskBufferStream::abort() noexcept {
    std::lock_guard<std::mutex> lock(_mutex);
    _aborted = true;
    _condition_read.notify_all();
    _condition_write.notify_all();
}
void AudioTaskBufferStream::clear() noexcept {
    std::lock_guard<std::mutex> lock(_mutex);
    _buffers = AudioTaskBufferList{};
    _current_size = 0;
    _condition_write.notify_all();
}
void AudioTaskBufferStream::flush_begin() noexcept {
    std::lock_guard<std::mutex> lock(_mutex);
    _flushing = true;
    _buffers = AudioTaskBufferList{};
    _current_size = 0;
...
void AudioTaskBufferStream::flush_end() noexcept {
    std::lock_guard<std::mutex> lock(_mutex);
    _flushing = false;
    _finished = false;
    _condition_read.notify_all();
