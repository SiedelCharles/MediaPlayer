...
public:
    ...
    /// @note Doesn't free any memory until the whole string has been discarded in all copies
    ///       of the Buffer.
...
public:
    ...
    [[nodiscard]] const std::deque<AudioTaskBuffer>& buffers() const { return _buffers; }
    ...
    [[nodiscard]] const AudioTaskBuffer& front() const;
...
public:
    /// @name Constructors
    /// @{
    AudioTaskBufferViewList() = default;
    ...
    /// @brief Access the underlying views
    [[nodiscard]] const std::deque<std::string_view>& views() const { return _views; }

    /// @brief append string_view
    void append(std::string_view str_view);

    /// @brief Discard the first `n` bytes of the string (does not require a copy or move)
    void remove_prefix(size_t n);

    /// @brief Is views empty
    [[nodiscard]] bool empty() const { return size() == 0; }
